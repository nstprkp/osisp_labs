...
int main(int argc, char* argv[])
{
    ...
    int n = scandir(path, &objects, NULL, NULL);

    if(flag.s) n = scandir(path, &objects, NULL, compare);

    for (int i = 0; i < n; i++) {
        if(flag.d && objects[i]->d_type == DT_DIR) printf("%s/%s\n", path, objects[i]->d_name);
        if(flag.f && objects[i]->d_type == DT_REG) printf("%s/%s\n", path, objects[i]->d_name);
        if(flag.l && objects[i]->d_type == DT_LNK) printf("%s/%s\n", path, objects[i]->d_name);
    }
