...
void child_birth(const char *file_name, const char *symbol, const char *child_path, char *envp[]) {
    pid_t pid = fork();
    if(pid < 0) {
        perror("fork");
        exit(1);
...
int main(int argc, char * argv[], char * envp[]) {
    ...
    qsort(env_vars, env_count, sizeof(char *), compare_env);
            ...
            for (int i = 0; envp[i] != NULL; i++) {
                ...
                *new_name = '\0';
                ...
                if (strcmp(CHILD_PATH, name) == 0) {
                    child_birth(argv[1], "*", new_name, envp);
                }
                free(name);
            }
        }
